    ...
    <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] overflow-hidden border border-slate-100 card-hover transition-all duration-500 group flex flex-col h-full shadow-sm hover:shadow-xl">
      <div className="relative h-24 md:h-32 overflow-hidden">
        ...
        <div className="absolute bottom-2 right-2">
          {isOpen ? (
            <span className="bg-emerald-500 text-white text-[8px] font-black px-2 py-1 rounded-lg uppercase tracking-widest flex items-center gap-1.5 shadow-lg">
      ...
      <div className="p-3 md:p-4 flex-grow flex flex-col">
        <div className="flex items-center gap-2 mb-1">
          <Link to={`/business/${business.id}`} className="block">
            <h3 className="text-xs md:text-lg font-black text-brand-teal-deep group-hover:text-brand-teal transition-colors leading-tight tracking-tight line-clamp-1">
              {business.name}
            </h3>
          </Link>
        </div>
        ...
        {business.segment && (
          <p className="text-brand-orange text-[8px] font-black uppercase tracking-widest mb-2">
            {business.segment}
          </p>
        )}
        ...
        {business.rating ? (
          <div className="flex items-center gap-1 mb-2">
            <div className="flex items-center text-amber-500">
              ...
              <span className="ml-1 text-slate-700 font-black text-[8px] md:text-xs">{business.rating}</span>
            </div>
            <span className="text-slate-400 text-[7px] md:text-[8px] font-bold">({business.reviewsCount})</span>
          </div>
        ) : null}
        ...
        <div className="space-y-1 mb-4">
          <a
            ...
            className="flex items-center text-slate-400 text-[8px] hover:text-brand-orange transition-colors group/address"
          >
            <ICONS.MapPin className="w-2.5 h-2.5 mr-1.5 flex-shrink-0 text-brand-orange group-hover/address:scale-110 transition-transform" />
        ...
        <div className="mt-auto pt-3 border-t border-slate-50">
          {(!business.isOfficial || (business.isOfficial && business.phone.replace(/\D/g, '').length >= 12)) && (
            <button
              ...
              className="w-full bg-brand-teal hover:bg-brand-teal-dark text-white font-black py-2.5 rounded-lg flex items-center justify-center transition-all shadow-lg shadow-brand-teal/10 uppercase tracking-widest text-[8px]"
            >
              <ICONS.MessageCircle className="w-3 h-3 mr-1.5" />
              WhatsApp
            </button>
          )}
        </div>
