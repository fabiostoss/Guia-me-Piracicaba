        ...
        {activeTab === 'spots' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {spots.map((spot, index) => (
              <div key={spot.id} className="bg-white rounded-[2.5rem] p-10 border border-slate-100 group hover:shadow-2xl transition-all duration-500 flex flex-col h-full animate-in fade-in zoom-in">
                <div className="flex justify-between items-start mb-10">
                  <div className="text-6xl font-black text-slate-100 group-hover:text-brand-teal/10 transition-colors">
                    {String(index + 1).padStart(2, '0')}
                  </div>
                  <span className="bg-slate-50 text-brand-teal text-[9px] font-black px-4 py-2 rounded-xl uppercase tracking-widest border border-brand-teal/10">
                    {spot.category}
                  </span>
                </div>
                ...
                <p className="text-slate-500 font-medium text-lg leading-relaxed mb-10">{spot.description}</p>
